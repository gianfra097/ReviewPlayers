<?php 
    ...
    if(isset($_REQUEST["term"])){       //Se è stato inizializzato il termine
        ...
        //Creo la query
        $sql_cerca = "SELECT * FROM news WHERE news_titolo LIKE ?";

        //mysqli_prepare, prepara un istruzione SQL per l'esecuzione
        if($stmt = mysqli_prepare($connection, $sql_cerca)){

            //Con mysqli stmt bind param, associo la variabile $param_term allo statement preparato come parametro. Utilizzo "s" perché è stringa
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) > 0){  //Se maggiore di 0 inizia il loop
                    ...
                    while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){   //Finchè ci sono risultati
                        ...
                        $imgcerca = $row["news_immagini"];
                        $news_id = $row["news_id"];  

                        ?>

                        <hr class="hr2" style="background: gray; color: gray; border: none; height: 0.5px; opacity: 0.5; width:97%;">

                        <a href="../include/lettura/singolanews.php?id=<?php echo "$news_id" ?>">
                            <div id="titoloeimgnews" style="display: flex; justify-content:flex-start;">
                                <img src="../images/imgpost/<?=$imgcerca;?>" width="20%" align="left" hspace="10"> 
                                <h3 class="titolo" style="text-transform: uppercase; padding-left: 10px;"> <b> <?php echo $row["news_titolo"]; ?> </b> </h3>                                    
                            </div> <br>
                        </a>
