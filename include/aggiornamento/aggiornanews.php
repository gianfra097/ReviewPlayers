 ...
 <body>
    ...
    <div class="content">
        <?php
            ...
            $sqlmod = "SELECT * FROM news WHERE news_id='$id'"; //Seleziono l'articolo con id uguale a quello che l'utente ha cliccato
            ...
            while($row = mysqli_fetch_assoc($risultatiagg)) {      //Fornisce i dati e li associa ai nomi delle colonne del database
                ...
                if(isset($_POST['aggiorna'])) {  //Se è stato premuto il bottone invia, aggiorna le variabili 
                   ...
                   //Inizializzo le variabili articolo e titolo
                    ...
                    $parolenews = strlen($newarticolo); //Conto le parole dell'articolo 

                    //Se l'utente ha inserito più di 10 righe allora può modificare l'articolo
                    if (empty($newarticolo)) {
                        echo ("<script LANGUAGE='JavaScript'> window.alert('Non puoi lasciare campi vuoti!'); window.location.href='../../pagine/news.php'; </script>");     
                    } else if($parolenews < 776){
                        echo ("<script LANGUAGE='JavaScript'> window.alert('Inserisci minimo 10 righe!'); window.location.href='../../pagine/news.php'; </script>");
                    } else{ 
                        $sqlaggiornarec = "UPDATE `news` SET `news_titolo` = '$newtitolo', `news_articolo` = '$newarticolo' WHERE `news_id`='$id'";  //Query per aggiornare la recensione*/
                        ...
                        if($risultatiagg) {
                            header("Location: ../../pagine/news.php");
                        }
                    }
                    ...
                    <b>Autore:</b><br>
                    <input name="autore" type="text" size="20" value="<?php echo $row["news_autore"];?>" disabled="disabled"> <br><br> <!-- Scrivo il nome dell'autore loggato-->
                    <b>Titolo:</b><br>
                    <input name="titolo" value="<?php echo $row["news_titolo"];?>" type="text" size="30"> <br><br>
                    <b>Articolo:</b> <font color="red"> Inserisci minimo 10 righe e massimo 15! </font> <br>
                    <textarea name="articolo" cols="100" rows="15" maxlength="1091"><?php echo $row["news_articolo"];?></textarea> <br><br>
