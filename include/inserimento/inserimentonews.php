    ...
    if(isset($_SESSION['username'])) { //Se l'utente ha effettuato l'accesso inserisci i dati nella tabella e/o avvia il form
        ...
        if(isset($_POST['submit']) && isset($_FILES['img'])) { //Se è stato cliccato invia e caricata l'immagine
            ...
            if(isset($_POST['articolo'])) {
                ...
                $parolenews = strlen($articolo);
            }

            //Se titolo, articolo o immagine sono stati lasciati vuoti visualizza un errore e refresha alla pagina news
            if($titolo == '' || $articolo == '' || $img_name == '') {
                echo ("<script LANGUAGE='JavaScript'> window.alert('Non puoi lasciare campi vuoti!'); window.location.href='news.php'; </script>"); 
            ...
            //Se l'estensione non è supportata, visualizza un errore e refresha alla pagina recensioni
                ...
                echo ("<script LANGUAGE='JavaScript'> window.alert('Formato immagine non supportato! I formati supportati sono: jpg, png e jpeg'); window.location.href='news.php'; </script>");

            } else if($parolenews < 776) {
                echo ("<script LANGUAGE='JavaScript'> window.alert('Inserisci minimo 10 righe!'); window.location.href='news.php'; </script>"); 

            } else { //Altrimenti, se i campi inseriti non sono vuoti, ed il file è supportato, procedi con l'inserimento

                move_uploaded_file($_FILES['img']['tmp_name'], $target);

                //Inseriamo i dati nella tabella news
                $sql = "INSERT INTO news (news_autore, news_titolo, news_articolo, news_immagini, news_data) VALUES ('$autore', '$titolo', '$articolo', '$img_name', now())";

                //Se l'inserimento ha avuto successo, inviamo una notifica
                if (@mysqli_query($connection, $sql) or die (mysqli_connect_error())){
                    echo ("<script LANGUAGE='JavaScript'> window.alert('Articolo inserito con successo!'); window.location.href='news.php'; </script>"); 
                }
            }
            ...
            <textarea name="articolo" cols="83" rows="10" maxlength="1091" placeholder="Minimo 10 righe e massimo 15!"></textarea> <br>
